...
weightAve3ROCfilter <- function(x, n = c(1, 3, 6), weights = c(1/3, 1/3, 1/3)){
  # same as weightAve3ROC() but filters negative ROCs
  ...
  if((sum(weights) != 1) || (length(n) != 3) || (length(weights) != 3)){
    ...
    roc3 <- ROC(x, n = n[3], type = "discrete")
    wave <- (roc1 * weights[1] + roc2 * weights[2] + roc3 * weights[3]) / sum(weights)
    wave <- ifelse(wave<0,NA,wave)
    rowRank(wave)
  }
}

weightAve3ROCstrict <- function(x, n = c(1, 3, 6), weights = c(1/3, 1/3, 1/3)){
  # same as weightAve3ROC() but filters negative ROCs
